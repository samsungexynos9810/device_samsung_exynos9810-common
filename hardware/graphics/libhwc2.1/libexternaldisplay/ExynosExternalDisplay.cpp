...
int ExynosExternalDisplay::openExternalDisplay()
{
    ...
    mClientCompositionInfo.mCompressed = false;
    mExynosCompositionInfo.mCompressed = false;
    mPlugState = true;
    ret = mResourceManager->distributeResourceSet();
    if (ret < 0)
        return ret;
...
bool ExynosExternalDisplay::checkHotplugEventUpdated()
{
    bool hpd_temp = 0;
    int sw_fd = open(mEventNodeName, O_RDONLY);
    char val;
    ...
    ALOGI("[%s] mDisplayId(%d), mIndex(%d), mHpdStatus(%d), hpd_temp(%d)", __func__, mDisplayId, mIndex, mHpdStatus, hpd_temp);

    if (mHpdStatus == hpd_temp)
        return false;
    else
        return true;
